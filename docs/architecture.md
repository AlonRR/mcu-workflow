...
*Resolved (revised 22 Jul 2026):* CI host is **Gitea Actions with self-hosted runners**, on the author's own private Gitea (`git.internal.example`). Self-hosted is required regardless of platform because the HIL stage needs physically attached hardware, so the runner must live on a machine with boards plugged into it; runner labels map directly onto the chip/instrument tags from Section 8. Gitea Actions is deliberately workflow-compatible with GitHub Actions, so the existing `ci-templates/github/` YAML ports over largely unchanged — the `ci-templates/` directory gains a `gitea/` variant rather than being rewritten. Because the pipeline lives in the contract-first CLI, the CI config stays a thin "check out → `mcuflow env up` → build/flash/test → upload report," which is what keeps this choice low-lock-in in both directions.

*Prerequisite, not yet met:* no `act_runner` is registered against the Gitea instance today (verified 22 Jul 2026 — Actions is enabled by Gitea's own default since 1.21, but there is no executor). Standing one up, and deciding whether it lives in an NAS LXC for build-only jobs or on a bench machine for HIL, is tracked in the homelab repo's `docs/todo.md` (O10).

*Why this changed:* the previous decision recorded "the user hosts on GitHub" as the reason. That was drift rather than intent — the standing rule is that **all personal projects live on the private Gitea, and nothing is pushed to a public remote**. The GitHub remote stays, deliberately, as a *publish* target the author pushes manually when the project is ready to be public; it is not the working remote and not the CI host.
