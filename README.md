...
hil run up workbench ports bridge debug mcp doctor env`. Add `--sim` for no hardware; drop it (and
...
src/            the runtime (one module per deliverable)
  ...
  mcpserver/        `mcuflow mcp`: expose the CLI verbs as MCP tools (needs .[mcp])
